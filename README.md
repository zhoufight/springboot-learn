...
一、基本内容
通常在项目中需要定时更新一些内容或者定时执行一些内容，这时候就需要用到调度任务，在spring boot中，调度任务使用起来非常方便。
在配置类中加上`@EnableScheduling`注解，或者在启动入口加上`@EnableScheduling`注解，然后在任务方法中添加`@Scheduled`即可完成调度的任务。
在`@Scheduled`中，有一个参数cron这里需要注重一下，这个参数用来设置任务什么时候执行。
cron表达式一般有5个或者6个空格隔开，分别表示6个或者7个的域，各个域分别有不同的含义，在spring boot中，cron通常是有6个域。
```
6个域依次表示：seconds、minutes、hours、day of months、months、day of week
```

每个域可能的值有以下：
|值|含义|
|-|-|
|数字|表示当前域的值，例如seconds域中的5表示时间为5秒的时候|
|,|表示枚举多个值，例如minutes域中1,2,3表示分钟为第1，2，3分钟时执行|
|-|表示范围，hours域中的1-3表示时间为1点到3点中执行|
|/|表示间隔时间，minute域中5/20表示从5分开始，每隔20分钟执行一次|
| * |表示任何值|
|? |表示任何值，作用在day of months和day of week中，如果这两个有一个使用 * ，另一个需要使用？

二、代码见文件夹2018-05-08/调度任务/demo
